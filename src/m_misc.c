...
extern	long	mousebfire;
extern	long	mousebstrafe;
extern	long	mousebforward;

extern	long	joybfire;
extern	long	joybstrafe;
extern	long	joybuse;
extern	long	joybspeed;

extern  int     viewwidth, viewheight;

/* rhandeev: differentiated mouse X and Y sensitivity */
long		mouseXSensitivity;
long		mouseYSensitivity;
...
extern int      maxblocks, minblocks;

#ifdef __DOSOUND__
extern	long	mb_used;
#endif

#if defined(__DOSOUND__) || defined(__DOMUSIC__)
#include "i_sound.h"
#endif
...
void M_LoadDefaults (void)
{
   ...
   */
  fprintf(stdout, "opening basedefault: %s\n", defaultfile);
  ...
  if (f)
      ...
      while (!feof(f))
	{
      ...
      fclose (f);
      if (screenblocks > maxblocks) screenblocks = maxblocks;
      if (screenblocks < minblocks) screenblocks = minblocks;
    }
}
