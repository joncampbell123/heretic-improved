...
#define finit_height	(screenheight-SBARHEIGHT)
static int f_x, f_y; /* location of window on screen */
static int f_w, f_h; /* size of window on screen */
static int lightlev; /* used for funky strobing effect */
#define fb	(screen) /* This used to be a variable?! */
static int amclock;
...
void AM_clearFB(int __attribute__((unused)) color)
{
#if 0
  int i, j;
  int dmapx;
  ...
  if (screenwidth != SCREENWIDTH || SCREENHEIGHT != screenheight) {
	  byte *scrptr = screen;
	  byte *lump = maplump;
	  byte *lumpend = maplump + SCREENWIDTH*(SCREENHEIGHT-SBARHEIGHT);
	  int remain;
	  int x, y;
	  
	  for (y = 0; y < screenheight-SBARHEIGHT; y++) {
		  for (x = 0; x+SCREENWIDTH <= screenwidth;
		       x += SCREENWIDTH) {
			  memcpy(scrptr, lump, SCREENWIDTH);
			  scrptr += SCREENWIDTH;
		  }
		  remain = (screenwidth-x);
		  if (remain) {
			  memcpy(scrptr, lump, remain);
			  scrptr += remain;
		  }
		  lump += SCREENWIDTH;
		  if (lump >= lumpend) {
...
#if 0
	  /* Draw background on sides of status bar too */
	  if (screenwidth > SCREENWIDTH) {
		  lump = maplump;
		  for (y = 0; y < SBARHEIGHT; y++) {
			  x = 0;
			  for (; x+SCREENWIDTH <= X_DISP; x += SCREENWIDTH) {
				  memcpy(scrptr, lump, SCREENWIDTH);
				  scrptr += SCREENWIDTH;
			  }
			  remain = (X_DISP-x);
	  ...
	  memcpy(screen, maplump, SCREENWIDTH*(SCREENHEIGHT-SBARHEIGHT));
  }
}
...
void AM_drawThings(int colors, int __attribute__((unused)) colorrange)
{
  int i;
  mobj_t *t;
...
void AM_Drawer(void)
{
  ...
  if((gameepisode < (ExtendedWAD ? 6 : 4)) && gamemap < 10)
    {
      MN_DrTextA(LevelNames[(gameepisode-1)*9+gamemap-1], 20, Y_BOTTOM+145);
    }
  /*   I_Update();   */
