...
void SB_Drawer(void)
{
  ...
  if(viewheight == screenheight && !automapactive)
  ...
  else
    {
      if(SB_state == -1)
	{
	    memset(screen+(screenheight-SBARHEIGHT)*screenwidth, 3,
		   screenwidth*SBARHEIGHT);
	  V_DrawPatch(0, Y_BOTTOM+158, PatchBARBACK);
	  if(players[consoleplayer].cheats&CF_GODMODE)
	    {
	      V_DrawPatch(16, Y_BOTTOM+167,
			  W_CacheLumpName("GOD1", PU_CACHE));
	      V_DrawPatch(287, Y_BOTTOM+167,
			  W_CacheLumpName("GOD2", PU_CACHE));
	    }
	  oldhealth = -1;
	}
      DrawCommonBar();
      if(!inventory)
	{
	  if(SB_state != 0)
	    {
				/* Main interface */
	      V_DrawPatch(34, Y_BOTTOM+160, PatchSTATBAR);
	      oldarti = 0;
	      oldammo = -1;
	      oldarmor = -1;
	      oldweapon = -1;
      ...
      else
	{
	  if(SB_state != 1)
	    {
	      V_DrawPatch(34, Y_BOTTOM+160, PatchINVBAR);
	    }
	  DrawInventoryBar();
	  SB_state = 1;
	}
...
void DrawCommonBar(void)
{
  ...
  V_DrawPatch(0, Y_BOTTOM+148, PatchLTFCTOP);
  V_DrawPatch(290, Y_BOTTOM+148, PatchRTFCTOP);
  ...
  if(oldhealth != HealthMarker)
      ...
      if(healthPos > 100)
	{
      ...
      V_DrawPatch(0, Y_BOTTOM+190, PatchCHAINBACK);
      V_DrawPatch(2+(healthPos%17), Y_BOTTOM+chainY, PatchCHAIN);
      V_DrawPatch(17+healthPos, Y_BOTTOM+chainY, PatchLIFEGEM);
      V_DrawPatch(0, Y_BOTTOM+190, PatchLTFACE);
      V_DrawPatch(276, Y_BOTTOM+190, PatchRTFACE);
      ShadeChain();
...
void DrawMainBar(void)
{
  ...
  if(ArtifactFlash)
    {
      ...
      V_DrawPatch(182, Y_BOTTOM+161, W_CacheLumpNum(W_GetNumForName("useartia")
  ...
  if(deathmatch)
      ...
      for(i = 0; i < MAXPLAYERS; i++)
	{
      ...
      if(temp != oldfrags)
	{
	  V_DrawPatch(57, Y_BOTTOM+171, PatchARMCLEAR);
  ...
  if(oldkeys != playerkeys)
	...
	{
	  V_DrawPatch(153, Y_BOTTOM+164,
		      W_CacheLumpName("ykeyicon", PU_CACHE));
	}
      if(CPlayer->keys[key_green])
	{
	  V_DrawPatch(153, Y_BOTTOM+172,
		      W_CacheLumpName("gkeyicon", PU_CACHE));
	}
      if(CPlayer->keys[key_blue])
	{
	  V_DrawPatch(153, Y_BOTTOM+180,
		      W_CacheLumpName("bkeyicon", PU_CACHE));
	}
      oldkeys = playerkeys;
      /* UpdateState |= I_STATBAR; */
    }
  ...
  if(oldammo != temp || oldweapon != CPlayer->readyweapon)
      ...
      if(temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
	{
	  ...
	  V_DrawPatch(111, Y_BOTTOM+172, W_CacheLumpName(
  ...
  if(oldarmor != CPlayer->armorpoints)
    {
      V_DrawPatch(224, Y_BOTTOM+171, PatchARMCLEAR);
...
void DrawInventoryBar(void)
{
  ...
  for(i = 0; i < 7; i++)
	...
	{
	  V_DrawPatch(50+i*31, Y_BOTTOM+160, W_CacheLumpName(
		  patcharti[CPlayer->inventory[x+i].type], PU_CACHE));
	  DrSmallNumber(CPlayer->inventory[x+i].count, 69+i*31, 182);
	}
    }
  V_DrawPatch(50+curpos*31, Y_BOTTOM+189, PatchSELECTBOX);
  if(x != 0)
    {
      V_DrawPatch(38, Y_BOTTOM+159,!(leveltime&4) ? PatchINVLFGEM1 :
		  PatchINVLFGEM2);
    }
  if(CPlayer->inventorySlotNum-x > 7)
    {
      V_DrawPatch(269, Y_BOTTOM+159,!(leveltime&4) ?
  ...
  if(!inventory)
    {
      if(CPlayer->readyArtifact > 0)
	{
	  V_DrawFuzzPatch(286, Y_BOTTOM+170, W_CacheLumpName("ARTIBOX",
						    PU_CACHE));
	  V_DrawPatch(286, Y_BOTTOM+170,
  ...
  else
    {
      ...
      for(i = 0; i < 7; i++)
	{
	  V_DrawFuzzPatch(50+i*31, Y_BOTTOM+168, W_CacheLumpName("ARTIBOX",
							PU_CACHE));
	  if(CPlayer->inventorySlotNum > x+i
	     && CPlayer->inventory[x+i].type != arti_none)
	    {
	      V_DrawPatch(50+i*31, Y_BOTTOM+168, W_CacheLumpName(
							patcharti[CPlayer->inventory[x+i].type], PU_CACHE));
	      DrSmallNumber(CPlayer->inventory[x+i].count, 69+i*31, 190);
	    }
	}
      V_DrawPatch(50+curpos*31, Y_BOTTOM+197, PatchSELECTBOX);
      if(x != 0)
	{
	  V_DrawPatch(38, Y_BOTTOM+167, !(leveltime&4) ? PatchINVLFGEM1 :
		      PatchINVLFGEM2);
	}
      if(CPlayer->inventorySlotNum-x > 7)
	{
	  V_DrawPatch(269, Y_BOTTOM+167, !(leveltime&4) ?
		      PatchINVRTGEM1 : PatchINVRTGEM2);
	}
    }
}
