...
void G_BuildTiccmd (ticcmd_t *cmd)
{
  ...
  /* rhandeev: added mouse invert option */
  if (mouseInvert == true)
    mousey = -mousey;

  /* rhandeev: added mouse look option */
  if (mouseLook == true)
    {
      static int mouse_sum = 0;
...
void G_DoSaveGame(void)
{
  ...
  if(cdrom)
    ...
    {
      sprintf(name, "%s"SAVEGAMENAME"%d.hsg", homedir, savegameslot);
    }
