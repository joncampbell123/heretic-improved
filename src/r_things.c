...
fixed_t		pspritescale, pspriteiscale, pspriteyscale;
...
/*
  ...
  =
  = Jonathan Campbell: So... you have a const char * array that is NOT NULL
  =                    terminated and who's length is determined by the
  =                    NUMSPRITES enum yet you scan until NULL anway?
  =                    I guess you got lucky with Watcom C/C++ back in the
  =                    day, because otherwise this would have prevented your
  =                    game from working!
  =
  =        FIX 2011-09-05: No longer scan until NULL. Use the NUMSPRITES
  =                        enum to know how many there are. This avoids a
  =                        segfault when compiled by GCC 4.6.x who apparently
  =                        does NOT ensure NULL data past the end of the
  =                        array.
  =
...
void R_InitSpriteDefs (char **namelist)
{
  ...
  numsprites = NUMSPRITES; /* NTS: global variable, used all over this code. */
                           /* FIXME: JMC, can you replace all instances with
			    *        the NUMSPRITES enum constant? */
...
{
  drawseg_t		*ds;
  short			clipbot[MAXSCREENWIDTH], cliptop[MAXSCREENWIDTH];
  int			x, r1, r2;
